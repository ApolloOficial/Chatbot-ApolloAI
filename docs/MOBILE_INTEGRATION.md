...
```text
Aplicativo Apollo → backend/gateway autenticado do Apollo → ApolloAI → MongoDB Atlas / Redis Cloud / provedor de IA
```
...
- [ ] **Configurar segredos de homologação.** Ativar `AUTH_REQUIRED=true`, definir token exclusivo, chave Groq/Google, URIs gerenciadas e rotacionar credenciais usadas durante a configuração.
