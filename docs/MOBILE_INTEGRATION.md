...
```text
Aplicativo Apollo → backend/gateway autenticado do Apollo → ApolloAI → MongoDB / Redis / Qdrant remotos / Amazon Bedrock
```
...
- [ ] **Configurar segredos de homologação.** Definir token exclusivo, chave do Qdrant, URIs gerenciadas e identidade IAM autorizada para Claude no Bedrock.
