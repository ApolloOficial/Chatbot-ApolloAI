...
Os preços de tokens permanecem em zero porque o cenário padrão usa o plano gratuito da Groq. O custo total inclui a infraestrutura estimada do Learner Lab: US$ 12,50/mês para 100 usuários semanais e US$ 20,10/mês para 1.000 usuários semanais, dentro do orçamento de US$ 50. Esses valores consideram EC2 em `us-east-1`, 8 GB de EBS gp3, um IPv4 público, até 1 GB no ECR e pequena reserva para logs; impostos, tráfego excedente e serviços remotos externos não estão incluídos. Para uma simulação diferente, ajuste `AWS_MONTHLY_COST_100_USERS`, `AWS_MONTHLY_COST_1000_USERS`, preços de tokens e `TECHNICIAN_HOURLY_COST`. As demais premissas ficam em `.env.example`: 5 mensagens por usuário, 700 tokens de entrada e 350 de saída por chamada, 4 agentes por solicitação, 1 consulta RAG e 75% de resolução.
